...
let UserCookie = ($.isNode() ? process.env.tksc : $.getdata('tksc')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
let host = 'api.tianzhikui.com';
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!(await Envs())){
            ...
            DoubleLog(`\n 作者仓库https://github.com/YourAhTzu/YourAhTzu.git`)
            await getVersion();
...
function sign(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/addons/shopro/user_sign/sign`,
            headers: {
                    'Host':host,
                    'token':ck[0],
                    },
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/YourAhTzu/YourAhTzu/main/tkzk.js`,
        }
